drop function if exists public.claim_team_session(text, text, text);
...
set search_path = ''
as $$
  select private.claim_team_session_impl(
    target_team_code,
    device_label,
    device_fingerprint
  );
...
grant execute on function private.claim_team_session_impl(text, text, text) to authenticated;
